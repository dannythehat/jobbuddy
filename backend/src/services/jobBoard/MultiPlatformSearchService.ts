...
export class MultiPlatformSearchService {
  ...
  async searchAllPlatforms(
    ...
    const fetchPromises = connections.map(conn =>
      jobFetchService.fetchFromProvider(userId, conn.provider_name, params)
        .catch(error => {
          console.error(`Error fetching from ${conn.provider_name}:`, error);
          return [];
        })
    );
    ...
    // Save search history
    await this.saveSearchHistory(userId, params, sortedJobs.length, premiumCount, connections);
    
    return {
      jobs: sortedJobs,
      total_count: sortedJobs.length,
      premium_count: premiumCount,
      providers_used: connections.map(c => c.provider_name),
      search_duration_ms: Date.now() - startTime,
      has_more: false
    };
  }
  ...
  private async saveSearchHistory(
    ...
    await pool.query(query, [
      ...
      connections.map(c => c.provider_name),
      totalResults,
      premiumResults
    ]);
  }
